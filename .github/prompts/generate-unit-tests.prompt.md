...
You are helping strengthen the test suite for pure logic in `src/Core/`. We use **xUnit**.

## Task
For the function or module I point you at:

1. Identify its **behaviours and inputs** (happy path, edge cases, error conditions).
2. Generate xUnit unit tests that cover:
   - the happy path,
...
- Prefer table-driven tests (`[Theory]` with `[InlineData]`) where inputs vary but logic is the same.
